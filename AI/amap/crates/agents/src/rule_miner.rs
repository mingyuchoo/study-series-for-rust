...
fn load_documents(paths: &[std::path::PathBuf]) -> String {
    paths.iter().filter_map(|p| std::fs::read_to_string(p).ok()).map(|t| amap_llm::pii::mask(&t)).collect::<Vec<_>>().join("\n\n")
}
...
impl AgentTask for RuleMinerAgent {
    ...
    async fn execute(&self, ctx: &AgentContext) -> Result<AgentResult, OrchestrationError> {
        let pack = context_pack(ctx, "business rules conditions fees interest", ContextBudget::default()).await?;
        let docs = load_documents(&ctx.config.documents);
        let prompt = format!("{}\n\n## Documents\n{}\n\nExtract all business rules.", pack.render(), if docs.is_empty() { "(none)".to_string() } else { docs.clone() });
        let req = LlmRequest::new(TaskKind::BusinessRuleExtraction, AgentRole::RuleMiner, "", prompt).with_run(ctx.run_id.0.clone());
        ...
        let second_opinion = if ctx.config.token_budget != u64::MAX {
            match ctx.llm.complete(req.with_provider(ModelProvider::Anthropic)).await {
        ...
        for (i, r) in rules_json.iter().enumerate() {
            ...
            let parses = amap_invariant::parse_expr(&condition).is_ok() && amap_invariant::parse_expr(&result).is_ok();
            if !parses {
                unparsable += 1;
            }
            let src = &r["source"];
            let location = SourceLocation::new(src["file"].as_str().unwrap_or("unknown"), src["start_line"].as_u64().unwrap_or(0) as u32, src["end_line"].as_u64().unwrap_or(0) as u32);
            let code_evidence = ctx.config.source_root.join(&location.file).exists();
            let in_document = r["in_document"].as_bool().unwrap_or(false) || (!docs.is_empty() && docs.to_lowercase().contains(&name.to_lowercase()));
            let evidence = EvidenceSources { code: code_evidence, document: in_document, production: false, inferred: !code_evidence };
            let mut signals = EvidenceSignals::default();
            if let Some(other) = &second_opinion {
                let mine = format!("{condition} => {result}");
                let theirs: Vec<String> = other["rules"].as_array().map(|a| a.iter().map(|x| format!("{} => {}", x["condition"].as_str().unwrap_or(""), x["result"].as_str().unwrap_or(""))).collect()).unwrap_or_default();
                let (agree, _) = n_version_agreement(&[mine.clone(), theirs.iter().find(|t| **t == mine).cloned().unwrap_or_default()]);
                if agree {
            ...
            let id = existing
                ...
                .find(|e| e.name == name)
                .map(|e| e.id.clone())
                .unwrap_or_else(|| RuleId::new(format!("BR-{}-{:06}", ctx.function_id.0.trim_start_matches("FN-").to_uppercase(), existing.len() + i + 1)));
            let rule = BusinessRule {
                ...
                db_entities: str_list(&r["db_entities"]).into_iter().map(DbEntityId::new).collect(),
                interfaces: str_list(&r["interfaces"]).into_iter().map(InterfaceId::new).collect(),
                observed_production_cases: 0,
                evidence,
                confidence,
                priority: priority_from(&r["priority"]),
                tags: if parses { vec![] } else { vec!["unparsable".into()] },
            };
            ctx.knowledge.add_relationship(Relationship::new(ctx.function_id.0.clone(), RelationKind::FunctionToRule, id.0.clone())).await?;
            ctx.knowledge.upsert_rule(rule).await?;
            ids.push(id.0);
            stored += 1;
        }
        ctx.emit("rules.mined", json!({ "count": stored, "unparsable": unparsable })).await;
        Ok(AgentResult::new(
...
pub fn rule_as_invariant(rule: &BusinessRule) -> Option<amap_invariant::Invariant> {
    ...
    Some(amap_invariant::Invariant { name: rule.id.0.clone(), guard: Some(guard), body })
}
