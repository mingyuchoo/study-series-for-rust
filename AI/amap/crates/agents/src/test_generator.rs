...
impl AgentTask for TestGeneratorAgent {
    ...
    async fn execute(&self, ctx: &AgentContext) -> Result<AgentResult, OrchestrationError> {
        ctx.authorize(&Principal::agent("test_generator"), "write_verification_test", &ctx.function_id.0, &ActionContext::default())?;
        ...
        let req = LlmRequest::new(TaskKind::TestGeneration, AgentRole::TestGenerator, "", pack.render()).with_run(ctx.run_id.0.clone());
        ...
        let mut count = 0;
        for (i, s) in j["scenarios"].as_array().cloned().unwrap_or_default().iter().enumerate() {
            let rule_ids: Vec<RuleId> = str_list(&s["rule_ids"]).into_iter().filter(|r| rules.iter().any(|x| x.id.0 == *r)).map(RuleId::new).collect();
            let priority = rule_ids.iter().filter_map(|r| rules.iter().find(|x| x.id == *r)).map(|x| x.priority).min().unwrap_or(Priority::P2);
        ...
        Ok(AgentResult::new(format!("generated {count} independent scenarios"), json!({ "scenarios": count, "provider": resp.provider })))
    }
}
