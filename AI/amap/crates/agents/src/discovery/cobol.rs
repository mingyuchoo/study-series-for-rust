...
pub fn analyze(file: &str, text: &str) -> AnalyzedFile {
    ...
    let proc_start = lines
        .iter()
        .position(|l| r.division.is_match(l) && l.to_uppercase().contains("PROCEDURE"))
        .unwrap_or(0);
    ...
    let mut program_entity = CodeEntity {
        id: prog_id.clone(),
        language: Language::Cobol,
        kind: EntityKind::Program,
        symbol: program.clone(),
    ...
    for (idx, (name, start)) in paragraphs.iter().enumerate() {
        ...
        entities.push(CodeEntity {
            id: id.clone(),
            language: Language::Cobol,
            kind: EntityKind::Paragraph,
            symbol: name.clone(),
            location: SourceLocation::new(file, *start, end),
            dependencies: deps,
            function_id: None,
            suspicious: false,
        });
    ...
    for c in r.copy.captures_iter(text) {
        ...
        entities.push(CodeEntity {
            id: SourceUnitId::new(format!("copybook::{cb}")),
            language: Language::Cobol,
            kind: EntityKind::Copybook,
            symbol: cb.clone(),
        ...
        program_entity
            .dependencies
            .push(SourceUnitId::new(format!("copybook::{cb}")));
    }
...
"#;
        ...
        assert!(a
            .relationships
            .iter()
            .any(|r| r.kind == RelationKind::SourceWritesDb && r.to == "LOAN_MASTER"));
