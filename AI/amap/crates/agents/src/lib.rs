...
pub async fn context_pack(ctx: &AgentContext, task_hint: &str, budget: ContextBudget) -> Result<ContextPack, OrchestrationError> {
    let mut engine = ContextEngine::new(Some(&ctx.config.source_root)).map_err(|e| OrchestrationError::Other(e.to_string()))?;
    engine.index(ctx.knowledge.as_ref()).await.map_err(|e| OrchestrationError::Other(e.to_string()))?;
    engine.build(ctx.knowledge.as_ref(), &ctx.function_id, task_hint, budget).await.map_err(|e| OrchestrationError::Other(e.to_string()))
}

/// Convert verification results into evidence-ledger records.
pub fn evidence_from_results(ctx: &AgentContext, results: &[VerificationResult], producer: &str) -> Vec<EvidenceRecord> {
    results
        .iter()
