...
pub fn run(cfg: &RunConfig, function_id: &FunctionId) -> Vec<VerificationResult> {
    ...
    for entry in WalkDir::new(&cfg.workspace).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file()) {
        let path = entry.path();
        let rel = path.strip_prefix(&cfg.workspace).unwrap_or(path).display().to_string();
        ...
        match Language::from_path(&rel) {
            Language::Python => {
                checked += 1;
                let out = Command::new("python3").args(["-m", "py_compile", &path.display().to_string()]).output();
                match out {
                    Ok(o) if o.status.success() => {}
                    Ok(o) => problems.push(format!("{rel}: {}", String::from_utf8_lossy(&o.stderr).trim())),
    ...
    vec![VerificationResult {
        ...
        differences: problems.iter().map(|p| Difference { path: "$".into(), expected: json!("compiles"), actual: json!(p), comparator: "static".into(), message: p.clone() }).collect(),
        explained: false,
        explanation: None,
        duration_ms: 0,
