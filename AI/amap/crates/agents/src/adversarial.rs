...
pub fn builtin_probes(base_input: &Value, base_state: &Value) -> Vec<(String, Value, Value)> {
    ...
    for path in &leaves {
        ...
        if let Some(Value::Number(n)) = get(base_input, path) {
            ...
            for (label, val) in [("negative", json!(-f.abs().max(1.0))), ("zero", json!(0)), ("max", json!(9007199254740991i64)), ("fraction", json!(f + 0.001))] {
                let mut v = base_input.clone();
                set(&mut v, path, val);
        ...
        if let Some(Value::String(s)) = get(base_input, path) {
            for (label, val) in [("empty", json!("")), ("unicode", json!(format!("{s}✓한글"))), ("long", json!("x".repeat(4096)))] {
                let mut v = base_input.clone();
                set(&mut v, path, val);
                probes.push((format!("{label}:{}", path.join(".")), v, base_state.clone()));
            }
            if s.contains('-') && s.len() >= 10 {
                for (label, date) in [("leap_day", "2024-02-29"), ("year_end", "2025-12-31"), ("month_end", "2026-02-28"), ("out_of_order", "1999-01-01")] {
                    let mut v = base_input.clone();
...
fn leaf_paths(v: &Value, prefix: Vec<String>) -> Vec<Vec<String>> {
    match v {
        Value::Object(m) => m.iter().flat_map(|(k, v)| {
...
impl AgentTask for AdversarialAgent {
    ...
    async fn execute(&self, ctx: &AgentContext) -> Result<AgentResult, OrchestrationError> {
        ...
        let Some(base) = behaviors.first() else {
            return Ok(AgentResult::new("no base behavior for adversarial probing", json!({ "scenarios": 0 })));
        };
        let rules = ctx.knowledge.rules_for(&ctx.function_id).await?;
        let mut count = 0;
        let mut store = |name: String, input: Value, state: Value, ctx: &AgentContext| {
            ...
            TestScenario {
                ...
                initial_state: state,
                input: json!({ "probe": name, "case": input }).get("case").cloned().unwrap_or(Value::Null),
        ...
        for (name, input, state) in builtin_probes(&base.input, &base.initial_state) {
            scenarios.push((name, store(String::new(), input, state, ctx)));
        }

        // LLM probes from a model different from the builder's.
        let builder_provider: Option<ModelProvider> = serde_json::from_value(ctx.input("build")["provider"].clone()).ok();
        let mut pack = context_pack(ctx, "adversarial edge cases", ContextBudget::default()).await?;
        pack.source.clear();
        let mut req = LlmRequest::new(TaskKind::AdversarialProbe, AgentRole::Adversarial, "", format!("{}\n\nBase input: {}", pack.render(), base.input)).with_run(ctx.run_id.0.clone());
        ...
        let llm_probes = match ctx.llm.complete(req.clone()).await {
            Ok(r) => r.json.and_then(|j| j["probes"].as_array().cloned()).unwrap_or_default(),
            Err(_) => match ctx.llm.complete(LlmRequest { provider: None, ..req }).await {
                Ok(r) => r.json.and_then(|j| j["probes"].as_array().cloned()).unwrap_or_default(),
                Err(e) => {
        ...
        for p in &llm_probes {
            if p["input"].is_object() {
                scenarios.push((p["name"].as_str().unwrap_or("llm").to_string(), store(String::new(), p["input"].clone(), p["initial_state"].clone(), ctx)));
            }
        }
        let n = scenarios.len();
        ...
        Ok(AgentResult::new(format!("generated {n} adversarial probes ({} from LLM)", llm_probes.len()), json!({ "scenarios": n, "llm_probes": llm_probes.len() })))
    }
}
