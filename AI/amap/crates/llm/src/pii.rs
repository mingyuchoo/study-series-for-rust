...
fn rules() -> &'static Rules {
    ...
    R.get_or_init(|| {
        Rules(vec![
            ...
            (Regex::new(r"\b01[016789]-?\d{3,4}-?\d{4}\b").unwrap(), "[PHONE]"),
            (Regex::new(r"\b\d{3}-\d{2}-\d{4}\b").unwrap(), "[SSN]"),
            (Regex::new(r"(?i)\b(?:sk-ant-|sk-|AKIA)[A-Za-z0-9_\-]{8,}").unwrap(), "[SECRET]"),
        ])
    })
}
...
mod tests {
    ...
    fn masks_common_pii() {
        ...
        let m = mask(s);
        assert!(m.contains("[RRN]") && m.contains("[CARD]") && m.contains("[EMAIL]") && m.contains("[SECRET]"));
