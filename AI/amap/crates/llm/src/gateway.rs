...
impl Default for GatewayConfig {
    fn default() -> Self {
        Self { cache_enabled: true, max_retries: 3, run_token_budget: 0, pii_filter: true }
    }
}
...
impl Gateway {
    pub fn new(router: Router, config: GatewayConfig) -> Self {
        Self { router, config, cache: Mutex::new(HashMap::new()), audit: Mutex::new(Vec::new()), run_usage: Mutex::new(HashMap::new()) }
    }
    ...
    pub fn run_usage(&self, run_id: &str) -> u64 {
        self.run_usage.lock().unwrap().get(run_id).copied().unwrap_or(0)
    }

    fn hash(req: &LlmRequest, provider: ModelProvider) -> String {
        let mut h = Sha256::new();
        h.update(format!("{:?}|{:?}|{}|{}|{:?}|{:?}", req.task, provider, req.system, req.prompt, req.schema, req.effort));
        hex::encode(h.finalize())
    }

    /// N-version reasoning: ask every configured provider independently (design §27).
    pub async fn n_version(&self, req: LlmRequest) -> Vec<(ModelProvider, Result<LlmResponse, LlmError>)> {
        let mut out = Vec::new();
...
impl LlmClient for Gateway {
    async fn complete(&self, mut req: LlmRequest) -> Result<LlmResponse, LlmError> {
        ...
        if let Some(run) = &req.run_id {
            if self.config.run_token_budget > 0 && self.run_usage(run) >= self.config.run_token_budget {
        ...
        let client = self.router.provider(provider).ok_or(LlmError::NotConfigured(provider))?;
        let mut attempt = 0;
        let resp = loop {
...
impl Gateway {
    fn record(&self, req: &LlmRequest, resp: &LlmResponse, key: &str, pii_masked: bool) {
        ...
        let cost = if resp.cached { 0.0 } else { (resp.input_tokens as f64 * pi + resp.output_tokens as f64 * po) / 1_000_000.0 };
        if let Some(run) = &req.run_id {
            *self.run_usage.lock().unwrap().entry(run.clone()).or_insert(0) += resp.input_tokens + resp.output_tokens;
        }
...
mod tests {
    ...
    async fn caches_and_masks() {
        ...
        let router = Router::new(RouterConfig::default()).with_provider(ModelProvider::Mock, mock.clone());
        let gw = Gateway::new(router, GatewayConfig::default());
        let req = LlmRequest::new(TaskKind::CodeReview, AgentRole::Reviewer, "", "customer 900101-1234567 review this").with_run("RUN-1");
