...
pub use pack::{ContextBudget, ContextPack, CodeSnippet};
...
impl ContextEngine {
    pub fn new(source_root: Option<&Path>) -> Result<Self, ContextError> {
        Ok(Self { lexical: LexicalIndex::in_memory()?, vector: InMemoryVectorIndex::default(), source_root: source_root.map(|p| p.to_path_buf()) })
    }
    ...
    pub async fn index(&mut self, store: &dyn KnowledgeStore) -> Result<usize, ContextError> {
        ...
        for r in &snap.rules {
            self.add(&r.id.0, "rule", &format!("{} {} {}", r.name, r.condition, r.result))?;
            n += 1;
        }
        for e in &snap.source_units {
            let body = self.read_snippet(&e.location).unwrap_or_default();
            self.add(&e.id.0, "source", &format!("{} {} {}", e.symbol, e.location, body))?;
            n += 1;
        }
        for b in &snap.behaviors {
            self.add(&b.id.0, "behavior", &format!("{} {}", b.input, b.legacy_output))?;
            n += 1;
        }
        self.lexical.commit()?;
        Ok(n)
    ...
    pub fn search(&self, query: &str, limit: usize) -> Vec<Hit> {
        ...
        for h in self.vector.search(query, limit) {
    ...
    pub fn read_snippet(&self, loc: &SourceLocation) -> Option<String> {
        ...
        Some(lines[start..end].iter().enumerate().map(|(i, l)| format!("{:>5} | {}", start + i + 1, l)).collect::<Vec<_>>().join("\n"))
    }

    /// Assemble the bounded context for a function (design §8: narrow bounded context per agent).
    pub async fn build(&self, store: &dyn KnowledgeStore, function_id: &FunctionId, task_hint: &str, budget: ContextBudget) -> Result<ContextPack, ContextError> {
        ...
        for unit in store.source_units_for(function_id).await? {
            let text = self.read_snippet(&unit.location).unwrap_or_else(|| format!("<{} {}>", unit.symbol, unit.location));
            source.push(CodeSnippet { location: unit.location.clone(), symbol: unit.symbol.clone(), text: amap_llm::pii::mask(&text) });
        }
        for r in &rules {
            for loc in &r.sources {
                ...
                if let Some(text) = self.read_snippet(loc) {
                    source.push(CodeSnippet { location: loc.clone(), symbol: r.name.clone(), text: amap_llm::pii::mask(&text) });
                }
            }
        }
        let schema = store.list_db_entities().await?.into_iter().filter(|d| rules.iter().any(|r| r.db_entities.iter().any(|x| x.0 == d.id.0 || x.0 == d.table))).collect();
        let interfaces = store.list_interfaces().await?.into_iter().filter(|i| rules.iter().any(|r| r.interfaces.contains(&i.id))).collect();
        ...
        let related = self.search(task_hint, budget.max_search_hits);
        let mut pack = ContextPack { function, requirements, rules, source, schema, interfaces, behaviors, tests, decisions, related, truncated: false, estimated_tokens: 0 };
