...
impl RunSpec {
    pub fn load(path: &Path) -> anyhow::Result<Self> {
        ...
        let abs = |p: &PathBuf| if p.is_absolute() { p.clone() } else { base.join(p) };
    ...
    pub fn function(&self) -> BusinessFunction {
        BusinessFunction { id: FunctionId::new(&self.function.id), name: self.function.name.clone(), domain: self.function.domain.clone(), priority: self.function.priority, description: self.function.description.clone() }
    }

    pub fn requirements(&self) -> Vec<Requirement> {
        self.requirements.iter().map(|r| Requirement { id: RequirementId::new(&r.id), function_id: FunctionId::new(&self.function.id), title: r.title.clone(), text: r.text.clone(), confidence: r.confidence }).collect()
    }
}
