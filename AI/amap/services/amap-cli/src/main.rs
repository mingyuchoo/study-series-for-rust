...
#[derive(Parser)]
#[command(name = "amap", version, about = "Autonomous Modernization Assurance Platform")]
struct Cli {
...
enum Cmd {
    ...
    Evidence {
        #[arg(long, default_value = "SELECT function_id, kind, priority, count(*) AS total, sum(CASE WHEN passed THEN 1 ELSE 0 END) AS passed FROM evidence GROUP BY function_id, kind, priority ORDER BY 1,2,3")]
        sql: String,
    },
...
async fn main() -> anyhow::Result<()> {
    ...
    amap_telemetry::init(&amap_telemetry::TelemetryConfig { service_name: "amap-cli".into(), json: settings.json_logs, otlp_endpoint: settings.otlp_endpoint.clone() });
    match cli.cmd {
        ...
        Cmd::Compare { spec, plugin, expected, actual } => {
            let spec = match spec {
                Some(p) => amap_comparator::ComparatorSpec::from_yaml(&std::fs::read_to_string(p)?)?,
            ...
            for p in plugin {
                ...
                engine.register_plugin(name, std::sync::Arc::new(amap_comparator::wasm::WasmComparator::from_file(name, path)?));
            }
        ...
        Cmd::Gate { certificate, metrics } => {
            let cert: FunctionCertificate = serde_json::from_str(&std::fs::read_to_string(certificate)?)?;
            let eq: EquivalenceMetrics = match metrics {
                Some(m) => serde_json::from_str(&std::fs::read_to_string(m)?)?,
                None => EquivalenceMetrics { all_total: cert.golden.total, all_passed: cert.golden.passed, ..Default::default() },
            };
        ...
        Cmd::Evidence { sql } => {
            let lake = if settings.lake.starts_with("s3://") { amap_evidence::EvidenceLake::open_s3(&settings.lake).await? } else { amap_evidence::EvidenceLake::open_local(&settings.lake).await? };
            let rows = lake.query(&sql).await?;
            println!("{}", serde_json::to_string_pretty(&rows)?);
            Ok(())
        }
        Cmd::Graph { snapshot } => {
            let snap: amap_knowledge::KnowledgeSnapshot = serde_json::from_str(&std::fs::read_to_string(snapshot)?)?;
        ...
        Cmd::Discover { root } => {
            for (file, a) in amap_agents::discovery::analyze_tree(&root) {
                println!("== {file}: {} entities, {} db entities, {} relationships", a.entities.len(), a.db_entities.len(), a.relationships.len());
                for e in a.entities {
                    println!("  {:?} {} @ {} -> {}", e.kind, e.symbol, e.location, e.dependencies.iter().map(|d| d.0.rsplit("::").next().unwrap_or("").to_string()).collect::<Vec<_>>().join(", "));
                }
            }
            Ok(())
        }
...
async fn run(settings: Settings, spec_path: PathBuf, mock: bool, out: Option<PathBuf>) -> anyhow::Result<()> {
    let spec = RunSpec::load(&spec_path).with_context(|| format!("loading {}", spec_path.display()))?;
    ...
    println!("▶ AMAP run {} for {} ({}){}", ctx.run_id, spec.function.id, spec.function.name, if platform.mock { " [mock LLM]" } else { "" });
    let outcome = amap_agents::run_modernization(ctx.clone()).await?;

    println!("\n== Steps ==");
    for (i, r) in outcome.reports.iter().enumerate() {
        for s in &r.steps {
            println!("  [{i}] {:<16} {:<9} {:>6}ms  {}", s.name, format!("{:?}", s.status), s.duration_ms, s.summary);
        }
    }
    ...
    if let Some(gw) = &platform.gateway {
        ...
        println!("\n== LLM audit == {} calls, {} tokens, ${:.4}, {} cached", audit.len(), tokens, cost, audit.iter().filter(|a| a.cached).count());
    }
    ...
    if !platform.settings.lake.starts_with("s3://") {
        ...
        std::fs::write(lake_dir.join(format!("{}-snapshot.json", ctx.run_id)), serde_json::to_string_pretty(&snapshot)?)?;
        println!("knowledge snapshot: {}", lake_dir.join(format!("{}-snapshot.json", ctx.run_id)).display());
    }
    println!(
        "\n== Result == {} after {} repair iteration(s){}{}",
        if outcome.certified { "FE CERTIFIED ✅" } else { "NOT CERTIFIED ❌" },
        outcome.iterations,
        outcome.halted.as_ref().map(|h| format!(" — halted: {h}")).unwrap_or_default(),
        outcome.failed.as_ref().map(|f| format!(" — failed: {f}")).unwrap_or_default()
    );
    if let Some(o) = out {
...
fn print_gate(g: &GateReport) {
    println!("== Quality Gate for {} → {} ==", g.function_id, if g.certified { "CERTIFIED" } else { "BLOCKED" });
    for c in &g.checks {
        println!("  {} {:<34} required {:<12} actual {}", if c.passed { "✔" } else { "✘" }, c.kpi, c.required, c.actual);
    }
}
