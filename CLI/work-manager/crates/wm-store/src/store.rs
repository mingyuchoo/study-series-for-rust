...
use wm_core::generation::{Generation, GenerationId, GenerationStatus, SwitchEvent};
...
pub struct GenerationEntry {
    pub meta: Generation,
    /// The store directory (`.wm/store/0007-a1b2c3d`).
    pub dir: PathBuf,
}
...
impl GenerationEntry {
    ...
    pub fn payload(&self) -> PathBuf {
        Layout::generation_payload(&self.dir)
    }
}
...
impl Store {
    ...
    pub fn get(&self, id: GenerationId) -> Result<GenerationEntry> {
        ...
        let text = fs::read_to_string(&meta_path)
            .map_err(|_| Error::NoSuchGeneration(id.0))?;
        let meta: Generation = serde_json::from_str(&text)?;
        Ok(GenerationEntry { meta, dir })
    }
    ...
    /// The generation a bare `wm rollback` would select: the highest id below
    ...
    pub fn stage(&self, commit: Option<&str>) -> Result<StagedGeneration> {
        ...
        Ok(StagedGeneration { id, dir, payload })
    }
    ...
    pub fn commit(&self, staged: StagedGeneration, meta: Generation) -> Result<GenerationEntry> {
        ...
        let relative = Path::new("..").join("store").join(
            staged.dir.file_name().unwrap_or_default(),
        );
        symlink(&relative, &link).ctx(format!("linking {}", link.display()))?;

        Ok(GenerationEntry { meta, dir: staged.dir })
    }

    /// Discard a staged generation that never made it to `commit`.
    pub fn discard(&self, staged: StagedGeneration) {
    ...
    pub fn switch(&self, id: GenerationId, reason: &str) -> Result<()> {
        ...
        fs::rename(&temp, &current).inspect_err(|_| {
            let _ = fs::remove_file(&temp);
        })
