...
///
/// Recording this is what keeps `wm status` honest once a worktree can be run
...
impl fmt::Display for RunSource {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            RunSource::Generation { id } => write!(f, "generation {id}"),
            RunSource::Worktree { name } => write!(f, "worktree `{name}` (dev, unverified)"),
        }
    }
}
...
impl RunSource {
    pub fn is_dev(&self) -> bool {
        matches!(self, RunSource::Worktree { .. })
    }
}
...
pub struct RunState {
    ...
    /// False while a foreground `wm dev run` owns the terminal.
    #[serde(default)]
    pub detached: bool,
}
