...
pub enum Error {
    #[error("no work-manager.toml found in {0} or any parent directory (run `wm init`)")]
    ProjectNotFound(PathBuf),

    #[error("work-manager.toml is already present at {0}")]
    ...
    #[error("another `wm` process holds the project lock ({0})")]
    Locked(PathBuf),
