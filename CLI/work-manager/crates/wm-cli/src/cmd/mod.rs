...
pub struct Project {
    ...
    pub store: Store,
    pub root: PathBuf,
    /// Set when the current directory is inside `.wm/worktrees/<name>`.
    pub worktree: Option<String>,
}

impl Project {
    /// Find `work-manager.toml` from the current directory upwards. Standing
    /// inside a worktree resolves to the real project root, not the worktree.
    ...
    pub fn select_worktree(&self, from: Option<&str>) -> Option<String> {
        from.map(str::to_string).or_else(|| self.worktree.clone())
    }
    ...
    pub fn worktree_path(&self, name: &str) -> Result<PathBuf> {
        ...
        if !path.is_dir() {
            bail!("worktree `{name}` does not exist (see `wm dev list`)");
        }
        Ok(path)
    }
}
...
pub fn dispatch(command: Command) -> Result<ExitCode> {
    match command {
        Command::Init { preset, name, force } => init::run(preset, name, force),

        Command::Status => service::status(),

        Command::Dev(DevCommand::New { name, base }) => dev::new(&name, base.as_deref()),
        Command::Dev(DevCommand::List) => dev::list(),
        Command::Dev(DevCommand::Run { from, no_build, detach }) => {
            dev::run(from.as_deref(), no_build, detach)
        }
        Command::Dev(DevCommand::Rm { name, force }) => dev::remove(&name, force),

        Command::Build { from, note, switch } => lifecycle::build(from.as_deref(), note, switch),
        Command::Switch { generation } => lifecycle::switch(generation),
        Command::Rollback { to } => lifecycle::rollback(to),
        Command::Generations => lifecycle::generations(),
        Command::History => lifecycle::history(),
        Command::Gc { keep } => lifecycle::gc(keep),
