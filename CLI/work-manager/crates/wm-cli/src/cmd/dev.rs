...
use wm_runner::{DevOutcome, Pipeline, git};
...
pub fn new(name: &str, base: Option<&str>) -> Result<ExitCode> {
    ...
    println!("  wm build --from {name} --switch");
    Ok(ExitCode::SUCCESS)
}
...
pub fn run(from: Option<&str>, no_build: bool, detach: bool) -> Result<ExitCode> {
    ...
    let Some(name) = project.select_worktree(from) else {
        bail!(
            "not inside a worktree — pass `--from <name>`, or use `wm start` to run the \
             active generation"
        );
    };
    ...
    match pipeline.dev_run(&name, &path, !no_build, detach, lock)? {
        DevOutcome::Detached(state) => {
            ...
            println!("  logs with `wm logs`, stop with `wm stop`");
            Ok(ExitCode::SUCCESS)
        }
        DevOutcome::Exited { code } => {
            println!();
            if code != 0 {
            ...
            if let Ok(Some(id)) = project.store.current_id() {
                println!("  service slot is free — `wm start` runs generation {id} again");
            }
...
pub fn list() -> Result<ExitCode> {
    ...
    for entry in std::fs::read_dir(&dir)? {
        ...
        let commit: String = git::head_commit(&path)
            .unwrap_or_default()
            .chars()
            .take(8)
            .collect();
    ...
    if rows.is_empty() {
        println!("no worktrees (create one with `wm dev new <name>`)");
    ...
    for (name, branch, commit, dirty, here) in rows {
        ...
        // `wm dev run` or `wm build` would act on.
