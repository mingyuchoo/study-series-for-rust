...
//!
//! Every test drives the real `wm` binary against a throwaway project in the
...
use wm_core::run::RunState;

pub const WM: &str = env!("CARGO_BIN_EXE_wm");

static COUNTER: AtomicU32 = AtomicU32::new(0);

/// Builder for `work-manager.toml`.
///
...
impl Manifest {
    ...
    fn render(&self) -> String {
        let artifacts: Vec<String> =
            self.artifacts.iter().map(|p| format!("\"{p}\"")).collect();
        let mut text = format!(
            "[project]\nname = \"demo\"\n\n\
             [build]\ncmd = \"{}\"\n\n\
             [artifacts]\ninclude = [{}]\n\n\
...
impl Fixture {
    /// A directory with no manifest, for testing `wm init` and discovery failure.
    pub fn bare() -> Fixture {
        let n = COUNTER.fetch_add(1, Ordering::SeqCst);
        let root = std::env::temp_dir().join(format!("wm-it-{}-{n}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        Fixture { root }
    }

    pub fn new(run_cmd: &str) -> Fixture {
        Fixture::with(Manifest::new(run_cmd))
    }
    ...
    pub fn set_manifest(&self, manifest: &Manifest) {
        std::fs::write(self.root.join("work-manager.toml"), manifest.render()).unwrap();
    }
    ...
    pub fn worktree_dir(&self, name: &str) -> PathBuf {
        self.root.join(".wm/worktrees").join(name)
    }
    ...
    /// The payload directory of a generation, as `wm` laid it out.
    pub fn generation_payload(&self, id: u64) -> PathBuf {
        self.root.join(format!(".wm/generations/{id:04}/root"))
    }

    pub fn run_state(&self) -> Option<RunState> {
        let text = std::fs::read_to_string(self.root.join(".wm/run/state.json")).ok()?;
    ...
    pub fn wm(&self, args: &[&str]) -> Output {
        self.wm_in(&self.root, args)
    }

    pub fn wm_in(&self, cwd: &Path, args: &[&str]) -> Output {
        Command::new(WM)
            .args(args)
            .current_dir(cwd)
            .output()
            .expect("failed to run the wm binary")
    }

    /// Run `wm` and fail the test if it did not succeed.
    pub fn wm_ok(&self, args: &[&str]) -> Output {
        let output = self.wm(args);
        assert!(
