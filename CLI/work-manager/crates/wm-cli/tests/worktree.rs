//! End-to-end tests for `wm init` and the worktree commands, which are the only
...
fn init_detects_the_project_type() {
    ...
    let output = fixture.wm_ok(&["init"]);
    assert!(stdout(&output).contains("wrote"), "stdout: {}", stdout(&output));

    let manifest = fixture.read("work-manager.toml");
...
fn init_accepts_an_explicit_preset_and_name() {
    ...
    fixture.wm_ok(&["init", "--preset", "node", "--name", "checkout"]);
...
fn init_rejects_an_unknown_preset() {
    ...
    let output = fixture.wm(&["init", "--preset", "cobol"]);
...
fn init_will_not_overwrite_an_existing_manifest_by_accident() {
    ...
    let refused = fixture.wm(&["init"]);
    assert!(!refused.status.success());
    assert!(stderr(&refused).contains("--force"), "stderr: {}", stderr(&refused));
    // The original manifest survived.
    assert!(fixture.read("work-manager.toml").contains("stop_timeout_secs"));

    fixture.wm_ok(&["init", "--force", "--preset", "go", "--name", "svc"]);
    assert!(fixture.read("work-manager.toml").contains("go build"));
}
...
fn dev_new_creates_a_worktree_on_its_own_branch() {
    ...
    let output = fixture.wm_ok(&["dev", "new", "add-cache"]);
    assert!(stdout(&output).contains("worktree `add-cache`"), "stdout: {}", stdout(&output));

    let worktree = fixture.worktree_dir("add-cache");
    assert!(worktree.join("work-manager.toml").is_file(), "the checkout is populated");
...
fn dev_new_rejects_a_name_already_in_use() {
    let fixture = repo();
    fixture.wm_ok(&["dev", "new", "add-cache"]);

    let output = fixture.wm(&["dev", "new", "add-cache"]);

    assert!(!output.status.success());
    assert!(
        stderr(&output).contains("worktree `add-cache` already exists"),
        "stderr: {}",
        stderr(&output)
    );
}
...
fn dev_new_requires_a_git_repository() {
    ...
    assert!(!output.status.success());
    assert!(
        stderr(&output).contains("is not a git repository"),
        "stderr: {}",
        stderr(&output)
    );
}
...
fn dev_list_marks_the_worktree_you_are_standing_in() {
    let fixture = repo();
    fixture.wm_ok(&["dev", "new", "add-cache"]);
    fixture.wm_ok(&["dev", "new", "fix-login"]);

    let from_root = stdout(&fixture.wm_ok(&["dev", "list"]));
    ...
    let from_worktree = stdout(&fixture.wm_in(&worktree, &["dev", "list"]));
    let marked: Vec<&str> =
        from_worktree.lines().filter(|line| line.starts_with('*')).collect();
...
fn dev_rm_removes_the_worktree() {
    ...
    let output = fixture.wm_ok(&["dev", "rm", "add-cache"]);
    assert!(stdout(&output).contains("removed worktree"), "stdout: {}", stdout(&output));
    assert!(!fixture.worktree_dir("add-cache").exists());

    let missing = fixture.wm(&["dev", "rm", "add-cache"]);
    assert!(!missing.status.success());
    assert!(
        stderr(&missing).contains("worktree `add-cache` does not exist"),
        "stderr: {}",
        stderr(&missing)
    );
}
...
fn a_worktree_never_gets_a_state_directory_of_its_own() {
    ...
    let status = stdout(&fixture.wm_in(&worktree, &["status"]));
    ...
    assert!(
        status.contains(fixture.root.canonicalize().unwrap().to_str().unwrap()),
        "status: {status}"
    );
    assert!(!worktree.join(".wm").exists(), "a second state directory appeared");
}
...
fn building_inside_a_worktree_needs_no_target_flag() {
    ...
    let output = fixture.wm_in(&worktree, &["build"]);
...
fn building_from_the_root_targets_the_root() {
    ...
    let out = stdout(&fixture.wm_ok(&["build"]));
