//! End-to-end tests for `wm dev run`.
//!
...
//! source so `wm status` can still say what is running.
...
/// A project with one worktree that needs no git behind it: `wm dev run` only
/// requires the directory to exist, so the fixtures stay fast and hermetic.
...
fn runs_the_worktree_code_in_the_foreground() {
    ...
    std::fs::write(
        fixture.worktree_dir(WORKTREE).join("worktree.marker"),
        "hello from the worktree",
    )
    .unwrap();

    let output = fixture.wm(&["dev", "run", "--from", WORKTREE]);

    assert!(output.status.success(), "stderr: {}", stderr(&output));
    // The child inherits stdio, so its output really is the command's output.
    assert!(
        stdout(&output).contains("hello from the worktree"),
        "stdout: {}",
        stdout(&output)
    );
}
...
fn no_build_skips_the_build_stage() {
    ...
    let output = fixture.wm(&["dev", "run", "--from", WORKTREE, "--no-build"]);
...
fn defaults_to_the_worktree_you_are_standing_in() {
    ...
    let output = fixture.wm_in(&worktree, &["dev", "run"]);
...
fn rejects_an_unknown_worktree() {
    ...
    let output = fixture.wm(&["dev", "run", "--from", "no-such-worktree"]);
...
fn a_detached_run_records_the_worktree_as_the_running_source() {
    ...
    let output = fixture.wm(&["dev", "run", "--from", WORKTREE, "--detach"]);
    assert!(output.status.success(), "stderr: {}", stderr(&output));

    let state = fixture.run_state().expect("a detached run must record its state");
    assert_eq!(state.source, RunSource::Worktree { name: WORKTREE.to_string() });
    assert!(state.detached);
    assert!(state.source.is_dev());

    // `wm status` names the source rather than only reporting that a pid exists.
    let status = stdout(&fixture.wm(&["status"]));
    assert!(status.contains("worktree `add-cache` (dev, unverified)"), "status: {status}");

    let stopped = fixture.wm(&["stop"]);
...
fn a_dev_run_and_a_generation_hand_the_slot_back_and_forth() {
    ...
    let built = fixture.wm_ok(&["build", "--switch"]);
    ...
    let taken = fixture.wm_ok(&["dev", "run", "--from", WORKTREE, "--detach"]);
    assert!(
    ...
    let restored = fixture.wm_ok(&["switch", "--gen", "1"]);
    assert!(
        stdout(&restored).contains("stopping worktree `add-cache`"),
        "stdout: {}",
        stdout(&restored)
    );
    assert_eq!(
