//! End-to-end tests for the service commands: `wm start`, `wm stop`,
//! `wm restart`, `wm logs` and `wm status`.
...
fn status_describes_a_project_with_nothing_built_yet() {
    ...
    let status = stdout(&fixture.wm_ok(&["status"]));
...
fn commands_outside_a_project_say_so() {
    ...
    let output = fixture.wm(&["status"]);

    assert!(!output.status.success());
    assert!(
        stderr(&output).contains("no work-manager.toml found"),
        "stderr: {}",
        stderr(&output)
    );
}
...
fn start_requires_an_active_generation() {
    ...
    let output = fixture.wm(&["start"]);

    assert!(!output.status.success());
    assert!(
        stderr(&output).contains("no generation is currently active"),
        "stderr: {}",
        stderr(&output)
    );
}
...
fn start_refuses_while_something_already_holds_the_slot() {
    ...
    assert!(err.contains("wm restart"), "stderr: {err}");
}
...
fn restart_replaces_the_running_process() {
    ...
    let restarted = fixture.wm_ok(&["restart"]);
    assert!(
        stdout(&restarted).contains("restarted generation 1"),
        "stdout: {}",
        stdout(&restarted)
    );
...
fn logs_show_what_the_service_wrote() {
    ...
    let logs = stdout(&fixture.wm_ok(&["logs"]));
...
fn status_names_the_active_generation_and_its_payload() {
    ...
    assert!(status.contains(".wm/store/0001-"), "status: {status}");
}
...
fn the_directory_flag_runs_against_another_project() {
    ...
    let output = Fixture::bare().wm_in(
        &elsewhere,
        &["-C", fixture.root.to_str().unwrap(), "generations"],
    );
