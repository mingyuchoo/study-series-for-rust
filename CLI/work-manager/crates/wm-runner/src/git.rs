...
pub fn is_repo(dir: &Path) -> bool {
    capture("git", &["rev-parse", "--git-dir"], dir).is_ok()
}

pub fn head_commit(dir: &Path) -> Option<String> {
    capture("git", &["rev-parse", "HEAD"], dir).ok()
}

pub fn current_branch(dir: &Path) -> Option<String> {
    capture("git", &["rev-parse", "--abbrev-ref", "HEAD"], dir).ok()
}
...
pub fn worktree_remove(repo: &Path, path: &Path, force: bool) -> Result<()> {
    let mut args: Vec<String> =
        vec!["worktree".into(), "remove".into(), path.to_string_lossy().into_owned()];
    if force {
...
pub fn worktree_prune(repo: &Path) {
    let _ = Command::new("git").args(["worktree", "prune"]).current_dir(repo).status();
}
