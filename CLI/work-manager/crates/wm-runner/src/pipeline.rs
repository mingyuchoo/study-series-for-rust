...
use wm_core::error::Result;
use wm_core::generation::{Generation, GenerationId, GenerationStatus};
use wm_core::run::{RunSource, RunState};
use wm_store::{GenerationEntry, ProjectLock, Store};

use crate::artifacts;
use crate::exec::run_stage;
use crate::git;
...
impl<'a> Pipeline<'a> {
    ...
    pub fn supervisor(&self) -> Supervisor {
        Supervisor::new(self.store.layout())
    }

    fn stop_timeout(&self) -> Duration {
        Duration::from_secs(self.config.run.stop_timeout_secs)
    }
    ...
    pub fn build(
        ...
        let collected =
            match artifacts::collect(source, &self.config.artifacts.include, &staged.payload) {
                Ok(list) => list,
                Err(err) => {
    ...
    pub fn activate(&self, id: GenerationId, reason: &str) -> Result<Activation> {
        ...
        let source = RunSource::Generation { id };
        let start_error = match supervisor.start_detached(
            &self.config.run,
            &entry.payload(),
            source.clone(),
        ) {
            Ok(state) => {
                match wait_until_healthy(&self.config.health, &entry.payload()) {
                    Ok(()) => {
                        self.store.set_status(id, GenerationStatus::Healthy)?;
                        return Ok(Activation::Healthy { id, pid: state.pid });
                    }
    ...
    /// `lock` is released as soon as the service slot is taken: a foreground run
    /// lasts as long as the developer keeps it open, and holding the project
    /// lock for all of that would block every other command in the meantime.
    pub fn dev_run(
        &self,
        name: &str,
        path: &Path,
        ...
        let supervisor = self.supervisor();
        let source = RunSource::Worktree { name: name.to_string() };
        ...
        if detach {
            ...
            drop(lock);
            Ok(DevOutcome::Exited { code: running.wait()? })
        }
    }
}
