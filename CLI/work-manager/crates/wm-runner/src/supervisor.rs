...
/// so `wm status` can always name what is running, not merely that something is.
///
...
impl Supervisor {
    pub fn new(layout: &Layout) -> Supervisor {
        Supervisor { state_file: layout.run_state(), log_file: layout.log_file() }
    }
    ...
    fn write_state(&self, state: &RunState) -> Result<()> {
        ...
        std::fs::write(&self.state_file, text)
            .ctx(format!("writing {}", self.state_file.display()))
    }
    ...
    /// outlives the `wm` invocation, with output appended to the log file.
    pub fn start_detached(
        ...
        let log_err = log
            .try_clone()
            .ctx(format!("cloning {}", self.log_file.display()))?;
        ...
        // Detach from the terminal's process group so Ctrl-C on `wm` does not
    ...
    /// Start attached to the terminal. Used by `wm dev run`: in the inner
    ...
    /// lock once the slot is taken, instead of holding it for the whole session.
    pub fn spawn_foreground(
        ...
        Ok(ForegroundRun { child, supervisor: self.clone() })
    }
