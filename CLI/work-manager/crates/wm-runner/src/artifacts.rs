    ...
    for rel in includes {
        ...
        if !src.exists() {
            return Err(Error::Config(format!(
                "declared artifact `{}` does not exist after the build",
                rel.display()
            )));
        }
...
fn copy_path(src: &Path, dest: &Path) -> Result<()> {
    ...
    if meta.file_type().is_symlink() {
        ...
        std::os::unix::fs::symlink(&target, dest)
            .ctx(format!("linking {}", dest.display()))?;
    } else if meta.is_dir() {
        ...
        fs::copy(src, dest)
            .ctx(format!("copying {} to {}", src.display(), dest.display()))?;
    }
    Ok(())
}

/// Total size of a directory tree, for `wm generations` reporting.
