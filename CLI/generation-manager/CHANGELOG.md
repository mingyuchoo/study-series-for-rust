...
- `gm dev new <name> [--base <ref>]` — `.gm/worktrees/<name>`에 git worktree와
  동명 브랜치를 만듭니다.
- `gm dev list` — worktree 목록. 현재 서 있는 곳을 `*`로 표시합니다.
- `gm dev rm <name> [--force]` — worktree 제거. 브랜치는 남깁니다.
- `gm dev run [--from N] [--no-build] [--detach]` — worktree 코드를 그 자리에서
...
- worktree 안에서 실행한 명령은 **원본 프로젝트 루트를 기준으로** 동작합니다.
  ...
  덕분에 `gm build`와 `gm dev run`이 대상 플래그 없이 현재 worktree를 씁니다.

#### 2단계 — 빌드·테스트·세대

- `gm build [--from N] [--note T] [--switch]` — 빌드와 테스트를 거쳐 산출물을
  불변 세대로 동결합니다. 어느 단계든 실패하면 세대 번호를 소비하지 않습니다.
- `gm switch [--gen N]` — 세대를 활성화하고 헬스체크로 검증합니다.
...
- `gm rollback [--to N]` — 이전 세대로 복귀합니다. 기본값은 활성 세대 바로
  아래 번호입니다.
...
- 프로젝트당 서비스 슬롯은 **하나**입니다. 슬롯을 차지한 주체를
  `.gm/run/state.json`에 pid와 함께 기록하므로, `gm status`가 활성 세대인지
  개발용 worktree인지 이름을 댈 수 있습니다.
- 슬롯을 가져가는 쪽은 무엇을 밀어냈는지 출력합니다.
- `gm start` / `gm stop` / `gm restart` / `gm logs [-n N]` — 활성 세대의
...
- `gm init [--preset P] [--name N] [--force]` — 매니페스트를 생성합니다.
  프리셋은 `rust` / `node` / `python` / `go` / `generic`이며, 미지정 시
  프로젝트 파일을 보고 고릅니다.
- `gm generations` (별칭 `gens`) — 세대 목록과 상태(`built` / `healthy` /
  `rejected`), 크기.
- `gm history` — 전환 이력과 사유 (`.gm/history.jsonl`).
- `gm gc [--keep N]` — 오래된 세대 삭제(기본 5). 활성 세대와 롤백 대상은 항상
  보존합니다.
