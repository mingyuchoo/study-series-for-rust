//! End-to-end tests for `gm dev run`.
//!
...
//! source so `gm status` can still say what is running.
...
/// A project with one worktree that needs no git behind it: `gm dev run` only
...
fn runs_the_worktree_code_in_the_foreground() {
    ...
    let output = fixture.gm(&["dev", "run", "--from", WORKTREE]);
...
fn no_build_skips_the_build_stage() {
    ...
    let output = fixture.gm(&["dev", "run", "--from", WORKTREE, "--no-build"]);
...
fn defaults_to_the_worktree_you_are_standing_in() {
    ...
    let output = fixture.gm_in(&worktree, &["dev", "run"]);
...
fn rejects_an_unknown_worktree() {
    ...
    let output = fixture.gm(&["dev", "run", "--from", "no-such-worktree"]);
...
fn a_detached_run_records_the_worktree_as_the_running_source() {
    ...
    let output = fixture.gm(&["dev", "run", "--from", WORKTREE, "--detach"]);
    assert!(output.status.success(), "stderr: {}", stderr(&output));
    ...
    // `gm status` names the source rather than only reporting that a pid exists.
    let status = stdout(&fixture.gm(&["status"]));
    assert!(status.contains("worktree `add-cache` (dev, unverified)"), "status: {status}");

    let stopped = fixture.gm(&["stop"]);
...
fn a_dev_run_and_a_generation_hand_the_slot_back_and_forth() {
    ...
    let built = fixture.gm_ok(&["build", "--switch"]);
    ...
    let taken = fixture.gm_ok(&["dev", "run", "--from", WORKTREE, "--detach"]);
    assert!(
    ...
    let restored = fixture.gm_ok(&["switch", "--gen", "1"]);
