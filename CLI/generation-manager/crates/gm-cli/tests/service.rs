//! End-to-end tests for the service commands: `gm start`, `gm stop`,
//! `gm restart`, `gm logs` and `gm status`.
...
fn status_describes_a_project_with_nothing_built_yet() {
    ...
    let status = stdout(&fixture.gm_ok(&["status"]));
...
fn start_refuses_while_something_already_holds_the_slot() {
    ...
    assert!(err.contains("gm restart"), "stderr: {err}");
}
...
fn restart_replaces_the_running_process() {
    ...
    let restarted = fixture.gm_ok(&["restart"]);
...
fn logs_show_what_the_service_wrote() {
    ...
    let logs = stdout(&fixture.gm_ok(&["logs"]));
...
fn the_directory_flag_runs_against_another_project() {
    ...
    let output = Fixture::bare().gm_in(&elsewhere, &["-C", fixture.root.to_str().unwrap(), "generations"]);
