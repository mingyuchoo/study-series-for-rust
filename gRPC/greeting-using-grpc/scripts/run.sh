#!/usr/bin/env bash
# greeting-using-grpc: 포맷 + 빌드 + 테스트 + 서버/클라이언트 실행
set -euo pipefail
...
Commands:
  ...
  server         cargo run -p greeting-server --bin server
  client         cargo run -p greeting-client --bin client
...
run_clippy() {
  step "Clippy (cargo clippy --workspace)"
  require_command cargo
...
build_release() {
  step "릴리스 빌드 (cargo build --workspace --release)"
  require_command cargo
...
run_server() {
  step "서버 실행 (cargo run -p greeting-server --bin server)"
  require_command cargo
...
run_client() {
  step "클라이언트 실행 (cargo run -p greeting-client --bin client)"
  require_command cargo
