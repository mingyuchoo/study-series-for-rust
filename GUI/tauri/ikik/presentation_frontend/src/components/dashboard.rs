...
pub struct IkikDashboardProps {
    pub items: Vec<IkikItem>,
    pub is_filtering: bool,
    pub on_open: EventHandler<IkikItem>,
    /// 단계 카드를 클릭하면 그 단계 탭으로 이동한다.
    pub on_kind_select: EventHandler<ItemKind>,
}
...
/// 대시보드 탭: 단계별 총 수량과 상태별 수량, Key Performance Indicator 달성
/// 현황을 한눈에 보여 준다. Key Performance Indicator는 주의가 필요한 지표가
/// 먼저 보이도록 달성률 낮은 순으로 나열한다.
pub fn IkikDashboard(props: IkikDashboardProps) -> Element {
    let t = *use_lang().read();
    ...
    let mut kpi_rows: Vec<KpiRow> = items
        ...
        .collect();
    // 달성률 낮은 순. 목표가 없어 달성률을 계산할 수 없는 Key Performance
    // Indicator는 뒤로 보낸다.
        ...
        section { class: "ikik-dashboard",
            ...
            div { class: "dash-cards",
                for (kind, total, by_status) in kind_cards {
                    // 카드 전체가 해당 단계 탭으로 가는 입구다 — 탭과 같은 동작.
                    button {
                        r#type: "button",
                        class: "dash-card",
                        onclick: move |_| props.on_kind_select.call(kind),
                        div { class: "dash-card-head",
                            span { class: "dash-card-kind", {kind_label(kind, t)} }
                            span { class: "dash-card-total", "{total}" }
                        }
                        p { class: "dash-card-desc", {kind_description(kind, t)} }
                        // 상태별 한 줄: 라벨 · 비율 막대 · 수량. 0인 상태도
                        // 빈 트랙으로 자리를 지켜 분포가 한눈에 비교된다.
                        div { class: "dash-status-rows",
                            for (index, status) in ItemStatus::ALL.into_iter().enumerate() {
                                {
                                    let count = by_status[index];
                                    let percent = if total > 0 { count * 100 / total } else { 0 };
                                    rsx! {
                                        div { class: "dash-status-row",
                                            span { class: "dash-status-label", "{status_label(status, t)}" }
                                            span { class: "dash-status-track",
                                                span { class: "dash-status-fill {status}", style: "width: {percent}%;" }
                                            }
                                            span { class: "dash-status-count", "{count}" }
                                        }
                                    }
                                }
                            }
