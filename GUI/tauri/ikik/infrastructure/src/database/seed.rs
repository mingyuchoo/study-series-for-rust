//! 첫 실행 시 비어 있는 데이터베이스를 IKIK 예시 계층으로 채운다.
//!
//! 멱등성: 항목이 하나라도 있으면 아무것도 하지 않으므로 앱을 여러 번
//! 실행해도 시드가 중복으로 들어가지 않는다.
//!
//! 시드 규모는 Identity 하나당 Key Result Area 넷, 그 아래는 부모
//! 하나당 자식 둘로, Identity 2 → Key Result Area 8 →
//! Income Generating Task 16 → Key Performance Indicator 32, 총 58개 항목이다.
...
fn seed_tree() -> [IdentitySeed; 2] {
    ...
    [
        IdentitySeed {
            title: "나는 경제적으로 자유로운 사람이다",
            description: "돈이 아니라 시간을 기준으로 선택하는 사람이 되어 간다",
            kras: [
                kra(
                    "온라인 강의 사업",
        ...
        IdentitySeed {
            title: "나는 꾸준히 단련하고 배우는 사람이다",
            description: "오래 일하고 오래 배울 수 있는 몸과 머리를 만드는 사람이 되어 간다",
            kras: [
                kra(
                    "근력 운동 루틴",
...
/// `metric`은 Key Performance Indicator 전용으로, (목표값, 단위, 집계
/// 방식)이다. 현재값은 0에서 시작한다.
async fn create<R>(
    repository: &R,
    kind: ItemKind,
...
/// 데이터베이스가 비어 있으면 IKIK 예시 계층(총 58개 항목)을 넣는다.
///
    ...
    R: ItemRepository + ?Sized,
{
    ...
    for (identity_position, identity) in seed_tree().into_iter().enumerate() {
        let identity_id = create(
            repository,
            ItemKind::Identity,
            None,
            identity.title,
            ...
            None,
            identity_position as i64,
        )
        .await?;

        for (kra_position, kra) in identity.kras.into_iter().enumerate() {
            let kra_id = create(
                repository,
                ItemKind::Kra,
                Some(identity_id),
...
mod tests {
    use super::*;
    use crate::SqliteIkikRepository;
    ...
    fn count_by_kind(items: &[domain::IkikItem], kind: ItemKind) -> usize { items.iter().filter(|item| item.kind == kind).count() }

    #[tokio::test]
    async fn seeds_empty_database_with_example_hierarchy() {
        ...
        assert_eq!(count_by_kind(&items, ItemKind::Identity), 2);
        ...
        for item in &items {
            match item.kind {
                | ItemKind::Identity => assert_eq!(item.parent_id, None),
                | _ => {
                    let parent_id = item.parent_id.expect("non-identity items should have a parent");
    ...
    async fn does_not_seed_when_user_data_exists() {
        let repository = repository().await;
        repository
            .create_item(IkikItem::new(NewIkikItem {
                kind: ItemKind::Identity,
                parent_id: None,
                title: "User identity".to_string(),
                description: None,
                target_value: None,
                current_value: None,
                unit: None,
        ...
        assert_eq!(items[0].title, "User identity");
    }
}
