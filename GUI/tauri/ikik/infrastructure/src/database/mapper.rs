...
pub fn row_to_item(row: &SqliteRow) -> Result<IkikItem, DomainError> {
    Ok(IkikItem {
        ...
        due_date: row
            ...
            .map(|date| NaiveDate::parse_from_str(&date, "%Y-%m-%d").map_err(|e| DomainError::DatabaseError(e.to_string())))
            .transpose()?,
