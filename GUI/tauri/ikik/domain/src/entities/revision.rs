...
pub struct ItemRevision {
    ...
    /// unit, status, aggregation, due_date. 화면 레이블은 프런트엔드가
    /// 매긴다.
    pub field: String,
...
pub fn diff_item_revisions(before: &IkikItem, after: &IkikItem, old_parent_title: Option<String>, new_parent_title: Option<String>) -> Vec<ItemRevision> {
...
mod tests {
    ...
    fn diff_records_only_changed_fields_and_skips_noise() {
        let before = IkikItem::new(NewIkikItem {
            kind: ItemKind::Kpi,
            parent_id: None,
        ...
        after.update(ItemPatch {
            ...
            due_date: Some(chrono::NaiveDate::from_ymd_opt(2026, 7, 31)),
            ..ItemPatch::default()
        });
        ...
        assert_eq!(fields, vec!["title", "target_value", "aggregation", "due_date"]);
        let due = revisions.iter().find(|revision| revision.field == "due_date").unwrap();
        assert_eq!(due.old_value, None);
        assert_eq!(due.new_value.as_deref(), Some("2026-07-31"));
