...
>
> 아래 내용은 Ollama 웹사이트의 디자인 시스템을 분석한 것으로, 이 주소록 앱(`presentation_frontend`)을 향후 리스타일링할 때 따를 **목표 비주얼 가이드**로 보관합니다. 현재 앱의 실제 스타일은 [`presentation_frontend/assets/styles.css`](presentation_frontend/assets/styles.css)에 정의되어 있으며, 아직 이 가이드를 반영하지 않았습니다. 이 문서의 토큰·컴포넌트 명세는 구현 시 참고용이며, 앱의 현재 동작을 설명하지 않습니다.
