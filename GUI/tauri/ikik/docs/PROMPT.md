...
- 2026-06-12: 잔여 중복 4건 정리. ① models/tree.rs의 조상 순회 3중 구현(parent_path·parent_chain·short_parent_path)을 walk_ancestors 하나로 통일 — 깊이 가드(순환 방어)와 끊긴 부모 처리가 한 곳이 됐다. ② 스토어의 create/update/delete가 공유하던 busy→호출→새로고침/오류 패턴을 mutate 헬퍼로 추출 — "변이 후엔 반드시 목록 새로고침" 규칙이 코드 한 곳에 산다. ③ 마감 칩 계산을 deadline::chip_for(item, lang)로 모아 트리·상세가 같은 한 줄을 쓴다. ④ 트리 드래그의 재배치 요청 조립을 app.rs에서 store.reparent(item, new_parent)로 이동 — 요청 필드가 늘어도 화면 코드는 변하지 않는다.
