...
impl ItemKind {
    /// IKIK 계층 순서. 트리 정렬과 탭 순서의 기준이 된다.
    pub const ALL: [ItemKind; 4] = [Self::Identity, Self::Kra, Self::Igt, Self::Kpi];
...
impl FromStr for ItemKind {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            | "igt" => Ok(Self::Igt),
            | "kpi" => Ok(Self::Kpi),
            | _ => Err(format!("Unsupported IKIK item kind: {value}")),
        }
    }
}
...
impl FromStr for ItemStatus {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            | _ => Err(format!("Unsupported IKIK item status: {value}")),
        }
    }
}
...
mod tests {
    ...
    fn kind_knows_allowed_hierarchy() {
        ...
        assert!(
            !ItemKind::Kpi.allows_parent(ItemKind::Kra),
            "Key Performance Indicator는 정확히 1단계 위인 Income Generating Task만 허용"
        );
