...
impl SqliteContactRepository {
    ...
    pub async fn init(&self) -> Result<(), sqlx::Error> {
        ...
        self.migrate_address_to_memo().await?;

        Ok(())
    }

    /// Renames a legacy `address` column to `memo` on databases created before the
    /// field was repurposed. Idempotent: only runs when `address` still exists and
    /// `memo` does not, so fresh databases and already-migrated ones are untouched.
    async fn migrate_address_to_memo(&self) -> Result<(), sqlx::Error> {
        let columns = sqlx::query("PRAGMA table_info(contacts)").fetch_all(&self.pool).await?;
        let column_names: Vec<String> = columns.iter().map(|row| row.get::<String, _>("name")).collect();

        let has_address = column_names.iter().any(|name| name == "address");
        let has_memo = column_names.iter().any(|name| name == "memo");

        if has_address && !has_memo {
            sqlx::query("ALTER TABLE contacts RENAME COLUMN address TO memo").execute(&self.pool).await?;
        }
...
impl ContactRepository for SqliteContactRepository {
    async fn create(&self, contact: Contact) -> Result<Contact, DomainError> {
        sqlx::query(
            r#"
            INSERT INTO contacts (id, name, email, phone, memo, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            "#,
        )
    ...
    async fn get_by_id(&self, id: Uuid) -> Result<Option<Contact>, DomainError> {
        let row = sqlx::query("SELECT id, name, email, phone, memo, created_at, updated_at FROM contacts WHERE id = ?")
            .bind(id.to_string())
    ...
    async fn get_all(&self) -> Result<Vec<Contact>, DomainError> {
        let rows = sqlx::query("SELECT id, name, email, phone, memo, created_at, updated_at FROM contacts ORDER BY name")
            .fetch_all(&self.pool)
            .await
            .map_err(|e| DomainError::DatabaseError(e.to_string()))?;
    ...
    async fn update(&self, contact: Contact) -> Result<Contact, DomainError> {
        sqlx::query(
            r#"
            UPDATE contacts 
            SET name = ?, email = ?, phone = ?, memo = ?, updated_at = ?
            WHERE id = ?
            "#,
        )
        .bind(&contact.name)
    ...
    async fn search(&self, query: &str) -> Result<Vec<Contact>, DomainError> {
        ...
        let rows = sqlx::query(
            r#"
            SELECT id, name, email, phone, memo, created_at, updated_at
            FROM contacts
            WHERE name LIKE ?1 ESCAPE '\'
               OR email LIKE ?1 ESCAPE '\'
               OR phone LIKE ?1 ESCAPE '\'
               OR memo LIKE ?1 ESCAPE '\'
            ORDER BY name
            "#,
        )
        .bind(&search_pattern)
...
mod tests {
    ...
    #[tokio::test]
    async fn init_migrates_legacy_address_column_to_memo_without_data_loss() {
        let pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
        ...
        // Recreate a pre-rename database: the column is still named `address`.
        sqlx::query(
            r#"
        ...
        let id = Uuid::new_v4();
        sqlx::query("INSERT INTO contacts (id, name, address, created_at, updated_at) VALUES (?, ?, ?, ?, ?)")
            .bind(id.to_string())
            .bind("Ada Lovelace")
            .bind("Seoul")
        ...
        let repository = SqliteContactRepository::new(pool);
        repository.init().await.expect("init should migrate the legacy column");

        // The old `address` value is preserved under the new `memo` field, and a
        // second init() is a no-op (idempotent).
        repository.init().await.expect("re-running init should be idempotent");

        let contact = repository
            .get_by_id(id)
            .await
            .expect("lookup should succeed")
            .expect("migrated contact should exist");
        assert_eq!(contact.memo, Some("Seoul".to_string()));
    }
}
