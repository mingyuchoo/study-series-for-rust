...
pub fn domain_error_to_api(error: DomainError) -> ApiError {
    match error {
        | DomainError::ItemNotFound => ApiError::new(ApiErrorKind::NotFound, "VVKIK 항목을 찾을 수 없습니다."),
        | DomainError::InvalidVvkikData(message) => ApiError::new(ApiErrorKind::Validation, message),
