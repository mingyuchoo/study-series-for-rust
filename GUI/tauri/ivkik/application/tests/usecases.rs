...
#[derive(Default)]
struct MockIvkikRepository {
    items: Mutex<HashMap<Uuid, IvkikItem>>,
    measurements: Mutex<HashMap<Uuid, Vec<KpiMeasurement>>>,
    revisions: Mutex<Vec<ItemRevision>>,
}

impl MockIvkikRepository {
...
impl IvkikRepository for MockIvkikRepository {
    ...
    async fn get_item_by_id(&self, id: Uuid) -> Result<Option<IvkikItem>, DomainError> { Ok(self.items.lock().unwrap().get(&id).cloned()) }

    async fn list_items(&self) -> Result<Vec<IvkikItem>, DomainError> {
        let mut items: Vec<IvkikItem> = self.items.lock().unwrap().values().cloned().collect();
...
fn draft(kind: ItemKind, parent_id: Option<Uuid>, title: &str) -> NewIvkikItem {
    NewIvkikItem {
        kind,
        parent_id,
...
fn kpi_draft(parent_id: Option<Uuid>, title: &str) -> NewIvkikItem {
    NewIvkikItem {
...
async fn create_rejects_invalid_parent_hierarchy() {
    ...
    let result = create.execute(draft(ItemKind::Kra, Some(identity.id), "Sales engine")).await;
...
async fn create_persists_valid_hierarchy() {
    ...
    let identity = create
        ...
        .expect("identity should be created");
    let vision = create
        .execute(draft(ItemKind::Vision, Some(identity.id), "Independent studio"))
        .await
...
async fn seeded_kpi(repository: &Arc<MockIvkikRepository>, aggregation: KpiAggregation) -> IvkikItem {
    ...
    let vision = create.execute(draft(ItemKind::Vision, Some(identity.id), "Independent studio")).await.unwrap();
