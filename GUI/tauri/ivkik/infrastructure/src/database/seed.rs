//! 첫 실행 시 비어 있는 데이터베이스를 IVKIK 예시 계층으로 채운다.
//!
...
/// 데이터베이스가 비어 있으면 IVKIK 예시 계층(총 62개 항목)을 넣는다.
///
    ...
    R: ItemRepository + ?Sized,
{
    ...
    for (value_position, identity) in seed_tree().into_iter().enumerate() {
        let value_id = create(
            repository,
            ItemKind::Identity,
            None,
        ...
        for (vision_position, vision) in identity.visions.into_iter().enumerate() {
...
mod tests {
    use super::*;
    use crate::SqliteIvkikRepository;
    ...
    fn count_by_kind(items: &[domain::IvkikItem], kind: ItemKind) -> usize { items.iter().filter(|item| item.kind == kind).count() }
    ...
    async fn does_not_seed_when_user_data_exists() {
        let repository = repository().await;
        repository
            .create_item(IvkikItem::new(NewIvkikItem {
