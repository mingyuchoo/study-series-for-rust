...
impl Lang {
    ...
    pub fn tagline(self) -> &'static str {
        pick(self, "Identity에서 Key Performance Indicator까지, 큰 그림을 실행과 피드백으로 연결합니다.", "From Identity to Key Performance Indicator — connect the big picture to action and feedback.")
    }

    pub fn search(self) -> &'static str { pick(self, "검색", "Search") }

    pub fn search_placeholder(self) -> &'static str { pick(self, "Identity, Vision, Key Result Area, Income Generating Task, Key Performance Indicator 검색...", "Search Identity, Vision, Key Result Area, Income Generating Task, Key Performance Indicator...") }
    ...
    /// 전환될 언어를 버튼에 보여 준다: 한국어 화면에는 "EN", 영어 화면에는 "한".
    ...
    pub fn confirm_delete_aria(self) -> &'static str { pick(self, "IVKIK 항목 삭제 확인", "Confirm IVKIK item deletion") }
    ...
    // ── 보드 탭 ──────────────────────────────────────────────
    pub fn tabs_aria(self) -> &'static str { pick(self, "IVKIK 단계 탭", "IVKIK stage tabs") }
    ...
    pub fn all_kpis(self) -> &'static str { pick(self, "전체 Key Performance Indicator", "All Key Performance Indicators") }

    pub fn kpi_levels_heading(self) -> &'static str { pick(self, "Key Performance Indicator 현재 수준", "Key Performance Indicator current levels") }
    ...
    pub fn dash_no_measurable(self) -> &'static str { pick(self, "달성률을 계산할 Key Performance Indicator가 없습니다.", "No Key Performance Indicators with a computable progress.") }
    ...
    pub fn no_kpis(self) -> &'static str { pick(self, "Key Performance Indicator가 없습니다.", "No Key Performance Indicators.") }
    ...
    pub fn form_new_title(self) -> &'static str { pick(self, "새 IVKIK 항목", "New IVKIK item") }
    ...
    pub fn current_value_hint(self) -> &'static str {
        pick(self, "현재값은 실적 기록을 추가하면 집계 방식에 따라 자동 계산됩니다.", "The current value is aggregated automatically as you add records.")
    }
    ...
    // ── 스토어 오류 ──────────────────────────────────────────
    pub fn err_load_items(self) -> &'static str { pick(self, "IVKIK 항목을 불러오지 못했습니다", "Failed to load IVKIK items") }
    ...
    pub fn no_records_yet_detail(self) -> &'static str {
        pick(self, "아직 기록이 없습니다. 아래에서 첫 실적을 기록해 보세요.", "No records yet — add the first one below.")
    }
    ...
    pub fn note_placeholder(self) -> &'static str {
        pick(self, "오늘의 메모 (선택) — 느낌, 배운 점, 컨디션 등을 일기처럼 남겨 보세요.", "Today's note (optional) — how it went, what you learned, like a journal.")
    }

    pub fn cmd_enter_hint(self) -> &'static str { pick(self, "⌘+Enter로도 기록", "⌘+Enter to record") }

    pub fn no_records_yet_panel(self) -> &'static str { pick(self, "아직 기록이 없습니다. 첫 실적을 기록해 보세요.", "No records yet — add the first one.") }
    ...
    pub fn tree_empty_hint(self) -> &'static str {
        pick(self, "아직 IVKIK 항목이 없습니다. 아래에서 Identity부터 추가해 보세요.", "No IVKIK items yet. Start by adding an Identity below.")
    }
    ...
    pub fn tree_flow_hint(self) -> &'static str {
        pick(
            self,
            "Identity → Vision → Key Result Area → Income Generating Task → Key Performance Indicator · 행을 끌어 새 상위 항목 위에 놓으면 이동합니다",
            "Identity → Vision → Key Result Area → Income Generating Task → Key Performance Indicator · drag a row onto a new parent to move it",
        )
    }

    pub fn add_identity(self) -> &'static str { pick(self, "+ Identity 추가", "+ Add Identity") }
