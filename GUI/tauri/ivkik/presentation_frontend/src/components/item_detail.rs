...
pub struct ItemDetailProps {
    ...
    /// 「수정」 버튼: 수정 폼으로 진입한다.
    pub on_edit: EventHandler<IvkikItem>,
...
struct DescendantListProps {
    ...
    items: Vec<IvkikItem>,
    depth: usize,
    on_navigate: EventHandler<IvkikItem>,
}
    ...
    rsx! {
        ul { class: "descendant-list",
            for child in children {
                {
                    let child_kind = child.kind.label();
                        ...
                        li { class: "descendant-node",
                            div { class: "descendant-row",
                                ...
                                if let Some(progress) = progress {
                                    ...
                                    span { class: "status-pill {child.status}", "{status_label(child.status)}" }
                                }
                            }
                            DescendantList {
