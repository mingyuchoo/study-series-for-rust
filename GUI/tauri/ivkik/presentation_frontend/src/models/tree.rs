...
pub fn sort_items(items: &mut [IvkikItem]) {
    items.sort_by(|a, b| a.kind.rank().cmp(&b.kind.rank()).then(a.position.cmp(&b.position)).then(a.title.cmp(&b.title)));
}

pub fn sorted_children(parent_id: &str, items: &[IvkikItem]) -> Vec<IvkikItem> {
    let mut children: Vec<IvkikItem> = items.iter().filter(|item| item.parent_id.as_deref() == Some(parent_id)).cloned().collect();
...
/// 없는 항목.
pub fn root_items(items: &[IvkikItem]) -> Vec<IvkikItem> {
    let ids: HashSet<&str> = items.iter().map(|item| item.id.as_str()).collect();
    let mut roots: Vec<IvkikItem> = items
        .iter()
...
pub fn parent_chain(item: &IvkikItem, items: &[IvkikItem]) -> Vec<IvkikItem> {
...
pub fn progress_text(item: &IvkikItem) -> Option<String> {
...
pub fn has_children(id: &str, items: &[IvkikItem]) -> bool { items.iter().any(|item| item.parent_id.as_deref() == Some(id)) }

/// 드래그한 항목을 `target` 위에 놓아 상위를 바꿀 수 있는지.
/// 자기 자신·현재 부모·계층 규칙에 어긋나는 단계는 제외한다.
/// (자손은 항상 하위 단계라 계층 규칙만으로 순환이 차단된다.)
pub fn is_valid_drop(dragged: &IvkikItem, target: &IvkikItem) -> bool {
...
mod tests {
    ...
    fn item(id: &str, kind: ItemKind, parent_id: Option<&str>, position: i64, title: &str) -> IvkikItem {
        IvkikItem {
            id: id.to_string(),
            kind,
